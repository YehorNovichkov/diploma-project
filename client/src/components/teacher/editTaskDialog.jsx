...
export function EditTaskDialog({ task, setTask }) {
    ...
    const form = useForm({
        ...
        defaultValues: {
            ...
            deadline: task.deadline ? new Date(task.deadline) : tomorrow,
            subjectQuery: '',
    ...
    const onSubmit = async (values) => {
        setUploading(true)
        await updateTask(task.id, values.name, values.description, values.deadline, values.classId, values.subjectId)
            .then((data) => {
                setTask(data)
                form.reset()
                setDialogOpen(false)
                toast.success('Завдання успішно змінено')
            })
            .catch((e) => {
                setError(e)
            })
